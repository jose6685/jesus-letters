...
class BackendAIService {
  ...
  buildFullPrompt(userInput) {
    ...
    // 以 AI_Prompts_Detailed.md 為核心系統提示
    const systemPrompt = (this.aiPrompts || '').trim()

    // 使用者上下文（最小必要）
    const contextBlock = `【使用者上下文】\n暱稱：${nickname || '未提供'}\n宗教信仰：${religion || '未提供'}\n我要提問：${topic || '未提供'}\n具體求況：${situation || '未提供'}`

    // 嚴格輸出為可解析 JSON（與解析器一致）
    const outputSchema = `【輸出格式】\n請僅輸出以下JSON（不得包含任何說明文字或代碼框）：\n{\n  \"jesusLetter\": \"600–800字中文內容，以耶穌第一人稱口吻，逐一關懷人、事、時、地、物、心情，並帶出安慰、勸勉、教導、歸正與神的愛\",\n  \"guidedPrayer\": \"600–800字中文禱告，開場包含邀請（例如：跟著一起唸或按播放鍵），正文涵蓋認罪悔改、求天父醫治、求天父安慰、求天父幫助、祝福與交託\",\n  \"biblicalReferences\": [\"至少3條經文或故事出處，來自四層熱門度抽樣（Top Tier / Mid Tier / Less Cited / Hidden Gems）\"],\n  \"coreMessage\": \"不超過40字的一句話總結與盼望\"\n}`

    const prompt = `這是AI的提示詞，請嚴格依據以下內容生成並僅輸出 JSON：\n\n${systemPrompt}\n\n${contextBlock}\n\n${outputSchema}`
